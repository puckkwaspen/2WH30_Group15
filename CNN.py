...
# shape (x,y,x) x: amount of RGB channels, y: height, z: width
img, label = dataset[0]
...
test_size = len(dataset) - train_size - val_size  # Remaining for the test set
...
val_dl = DataLoader(val_data, batch_size=batch_size * 2, shuffle=False, num_workers=4, pin_memory=True)
test_dl = DataLoader(test_data, batch_size=batch_size * 2, shuffle=False, num_workers=4, pin_memory=True)

# Print a message indicating the loader sizes
print(f"Train DataLoader contains {len(train_dl)} batches")
print(f"Validation DataLoader contains {len(val_dl)} batches")
